...
function Freelance({ content, lang }: CurriculumLanguage) {
	return (content &&
		<div>
			<h3>{byLang(content.freelanceTitle, lang)}</h3>
			{sortByDates(content.freelance).map((p: professional, pi) => (
				<div key={pi} className="keep-together freelance">
					...
					<h5>
						<span className="copy-me" onClick={copyContent}>{byLang(p.role, lang)}</span>
						<small className="copy-me" onClick={copyContent}>
